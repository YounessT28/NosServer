...
namespace OpenNos.GameObject
{
    public class ItemInstance : ItemInstanceDTO
    {
        ...
        public string GenerateInventoryAdd()
        {
            switch (Type)
            {
                case InventoryType.Equipment:
                    return $"ivn 0 {Slot}.{ItemVNum}.{Rare}.{(Item.IsColored ? Design : Upgrade)}.{SpStoneUpgrade}.{RuneUpgrade}";
        ...
        public string GenerateSlInfo(ClientSession session = null)
        {
            ...
            if (ItemVNum == 4485 || ItemVNum == 4437 || ItemVNum == 4416 || ItemVNum == 4532 || ItemVNum == 8378)
            {
                morphUpdate = 30;
            }

            foreach (Skill ski in ServerManager.GetAllSkill().Where(ski => ski.Class == Item.Morph + morphUpdate && ski.LevelMinimum <= SpLevel).OrderBy(s => s.SkillVNum))
            {
