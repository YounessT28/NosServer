...
namespace OpenNos.GameObject
{
    public class Character : CharacterDTO
    {
        ...
        public Character(CharacterDTO input) : this()
        {
            ...
            BattleTowerLastStage = input.BattleTowerLastStage;
        }
        ...
        {
            return $"titinfo 1 {CharacterId} {ViewTitle} {EffectTitle}";
        }
        ...
        public void GenerateTitle()
        {
            ...
            foreach (var title in DAOFactory.CharacterTitlesDAO.LoadByCharacterId(CharacterId).ToList())
            {
                ...
                if (title.TitleId == ViewTitle && title.TitleId == EffectTitle)
                {
                    str += $"{titvnum}.7 ";
                }
                else if (title.TitleId != ViewTitle && title.TitleId == EffectTitle)
                {
                    str += $"{titvnum}.5 ";
                }
                else if (title.TitleId == ViewTitle && title.TitleId != EffectTitle)
...
#warning TODO: Change sheep score
        ...
        public bool AddTattoo(short skillVNum, int TattooUpgrade, bool IsTattoo)
        {
        ...
        public void CharacterLife()
        {
            ...
            else
            {
                ...
                #region Battle Tower Reward

                switch (Session.Character.BattleTowerStage)
                {
                    /*
                    case 0:
                        if (Session.Character.MapId == 30007)
                        {
                    ...
                    */
                    case 7:
                        if (Session.Character.MapId == 30007 && BattleTowerLastStage == 0)
                        {
                            if (Session.Character.BattleTowerLastStage == 1)
                            {
                                Session.SendPacket("msg 4 You already claimed your reward!");
                                return;
                            }

                            Session.Character.BattleTowerLastStage = 1;
                            Session.Character.GiftAdd(5976, 10);
                            Session.SendPacket("msg 4 You received the Battle Coins in exchange for reaching Stage 7!");
                            ServerManager.Instance.ChangeMap(Session.Character.CharacterId, 1, 80, 116);
                        }
                        break;
                }
                ...
                if (Session.Character.BattleTowerEXP >= 10 && Session.Character.BattleTowerStage == 0)
                {
                    Session.Character.BattleTowerStage += 1;
                    Session.Character.BattleTowerEXP = 0;
                    Session.SendPacket("msg 4 You can now enter Battle Tower Stage 2");
                }

                if (Session.Character.BattleTowerEXP >= 20 && Session.Character.BattleTowerStage == 1)
                {
                    Session.Character.BattleTowerStage += 1;
                    Session.Character.BattleTowerEXP = 0;
                    Session.SendPacket("msg 4 You can now enter Battle Tower Stage 3");
                }

                if (Session.Character.BattleTowerEXP >= 40 && Session.Character.BattleTowerStage == 2)
                {
                    Session.Character.BattleTowerStage += 1;
                    Session.Character.BattleTowerEXP = 0;
                    Session.SendPacket("msg 4 You can now enter Battle Tower Stage 4");
                }

                if (Session.Character.BattleTowerEXP >= 80 && Session.Character.BattleTowerStage == 3)
                {
                    Session.Character.BattleTowerStage += 1;
                    Session.Character.BattleTowerEXP = 0;
                    Session.SendPacket("msg 4 You can now enter Battle Tower Stage 5");
                }

                if (Session.Character.BattleTowerEXP >= 160 && Session.Character.BattleTowerStage == 4)
                {
                    Session.Character.BattleTowerStage += 1;
                    Session.Character.BattleTowerEXP = 0;
                    Session.SendPacket("msg 4 You can now enter Battle Tower Stage 6");
                }

                if (Session.Character.BattleTowerEXP >= 240 && Session.Character.BattleTowerStage == 5)
                {
                    Session.Character.BattleTowerStage += 1;
                    Session.Character.BattleTowerEXP = 0;
                    Session.SendPacket("msg 4 You can now enter Battle Tower Stage 7");
                }
                #endregion
                ...
                if (Inventory != null)
                {
                    for (short i = 0; i < 17; i++)
                    {
                        ...
                        if (item != null)
                        {
                            ...
                            eqlist += $" {i}.{item.Item.VNum}.{item.Rare}.{(item.Item.IsColored ? item.Design : item.Upgrade)}.0.0";
                        }
                    }
                }
            });
        ...
        public string GenerateExts() => $"exts 0 {48 + ((HaveBackpack() ? 1 : 0) * 12) + ((HaveNewBackpack() ? 1 : 0) * 60)} {48 + ((HaveBackpack() ? 1 : 0) * 12) + ((HaveNewBackpack() ? 1 : 0) * 60)} {48 + ((HaveBackpack() ? 1 : 0) * 12) + ((HaveNewBackpack() ? 1 : 0) * 60)}";
        ...
        public string GenerateGidx()
        {
            ...
            return
                ...
                //$"gidx 1 {CharacterId} {Family.FamilyId} {Family.Name}({Language.Instance.GetMessageFromKey(FamilyCharacter.Authority.ToString().ToUpper())}) {Family.FamilyLevel}" : $"gidx 1 {CharacterId} -1 - 0";
            //  gidx 1 1601949 275.916 Dothrakis 10 0|0|0
        }
        ...
        public void GenerateKillBonus(MapMonster monsterToAttack, BattleEntity Killer)
        {
            ...
            lock (_syncObj)
            {
                ...
                Session.Character.Reputation += 50;

                if (MapId == 30000 && Session.Character.BattleTowerStage == 0)
                {
                    Session.Character.BattleTowerEXP += 1;
                }
                else if (MapId == 30001 && Session.Character.BattleTowerStage == 1)
                {
                    Session.Character.BattleTowerEXP += 2;
                }
                else if (MapId == 30002 && Session.Character.BattleTowerStage == 2)
                {
                    Session.Character.BattleTowerEXP += 2;
                }
                else if (MapId == 30003 && Session.Character.BattleTowerStage == 3)
                {
                    Session.Character.BattleTowerEXP += 3;
                }
                else if (MapId == 30004 && Session.Character.BattleTowerStage == 4)
                {
                    Session.Character.BattleTowerEXP += 3;
                }
                else if (MapId == 30005 && Session.Character.BattleTowerStage == 5)
                {
                    Session.Character.BattleTowerEXP += 4;
                }
                else if (MapId == 30006 && Session.Character.BattleTowerStage == 6)
                {
                    Session.Character.BattleTowerEXP += 5;
                }
        ...
        public string GenerateScpStc() => $"sc_p {(MaxMateCount - 10) / 10} {MaxPartnerCount - 3}";
                ...
                foreach (ItemInstance inv in Inventory.GetAllItems())
                {
                    switch (inv.Type)
                    {
                        case InventoryType.Equipment:
                            ...
                            {
                                inv0 += $" {inv.Slot}.{inv.ItemVNum}.{inv.Rare}.{(inv.Item.IsColored ? inv.Design : inv.Upgrade)}.0.{inv.RuneUpgrade}";
                            }
                            break;
