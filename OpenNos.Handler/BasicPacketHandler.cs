...
namespace OpenNos.Handler
{
    public class BasicPacketHandler : IPacketHandler
    {
        ...
        {
            if (DAOFactory.CharacterTitlesDAO.LoadByCharacterId(Session.Character.CharacterId).Any(s => s.TitleId == titEqPacket.TitleId))
            {
                switch (titEqPacket.Type)
                {
                    case 1:
                        if (Session.Character.ViewTitle == titEqPacket.TitleId)
                        {
                            Session.Character.ViewTitle = 0;
                        }
                        else if (Session.Character.ViewTitle != titEqPacket.TitleId)
                        {
                            Session.Character.ViewTitle = titEqPacket.TitleId;
                        }
                        break;

                    case 2:
                        if (Session.Character.EffectTitle == titEqPacket.TitleId)
                        {
                            Session.Character.EffectTitle = 0;
                        }
                        else if (Session.Character.EffectTitle != titEqPacket.TitleId)
                        {
                            Session.Character.EffectTitle = titEqPacket.TitleId;
                        }
                        break;
                }
                        ...
                        && Session.Character.PositionX <= s.SourceX + 1) is Portal portal)
            {
                switch (portal.Type)
                {
                    ...
                    case (sbyte)PortalType.TimeSpace:
                        if (portal.DestinationMapId == 30001 && Session.Character.BattleTowerStage == 0)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
                        if (portal.DestinationMapId == 30002 && Session.Character.BattleTowerStage == 1)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
                        if (portal.DestinationMapId == 30003 && Session.Character.BattleTowerStage == 2)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
                        if (portal.DestinationMapId == 30004 && Session.Character.BattleTowerStage == 3)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
                        if (portal.DestinationMapId == 30005 && Session.Character.BattleTowerStage == 4)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
                        if (portal.DestinationMapId == 30006 && Session.Character.BattleTowerStage == 5)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
                        if (portal.DestinationMapId == 30007 && Session.Character.BattleTowerStage == 6)
                        {
                            Session.SendPacket("msg 3 You can't enter this Stage yet!");
                            return;
                        }
        ...
        public void StartGame(GameStartPacket gameStartPacket)
        {
            ...
            Session.SendPacket($"msg 3 ========== NosTale Ventus ==========");
            Session.SendPacket($"Welcome, {Session.Character.Name} - Enjoy your stay!");
